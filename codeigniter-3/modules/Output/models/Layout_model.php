...
class Layout_model extends CI_Model {
	...
	public function view_content( $content_id, $view_data = array() ) {
		if( empty( $content_id ) )
			return $this->view_contents( $view_data );

		if( empty( $this->views['contents'] ) || empty( $view_data )  )
			return false;

		if( array_key_exists( $content_id, $this->views['contents'] ) )
			return false;
	...
	public function remove_content( $content_id, $view_file_path, $priority = 50 ) {
		$this->views['contents'] = $this->_remove( 'content', $this->views['contents'], $content_id, $view_file_path, $priority );
	...
	public function row() {
		return $this->Framework->row();
	} // function

	public function column( $size = 'xs', $grid_columns ) {
		return $this->Framework->column( $size, $grid_columns );
	} // function
